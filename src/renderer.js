...
function updateConnectionIndicator(status) {
  ...
  el.connStatusWrap.title = tooltip;
}
...
async function saveSettings() {
  ...
  appLogsExpanded = !Boolean(result.settings?.ui?.logsCollapsed !== false);
  updateLogsToggle();
...
function buildCommandPayload() {
  ...
  if (command === "discount") {
    return {
      command,
      args: {
        target: String(el.cmdTarget.value || "").trim(),
...
function refreshCommandInputs() {
  ...
  const needsTarget = command === "who" || command === "discount" || command === "discount_cancel";
  const needsValue = command === "discount";
  const needsDuration = command === "discount";
  ...
  if (command === "who") {
    ...
    el.cmdTarget.placeholder = "phone / nickname / uuid";
...
async function runBotCommand() {
  ...
  const result = await window.api.botConsoleRunCommand(payload);
  if (!result?.ok) {
